...
async function verifySignedEvent(event) {
  try {
    ...
    if (bytesToHex2(hash) !== event.id)
      return false;
    return schnorr.verify(hexToBytes2(event.sig), hash, hexToBytes2(event.pubkey));
  } catch {
    return false;
  }
}
...
function sha256HexSync(text) {
  return bytesToHex3(sha2562(textEncoder.encode(text)));
}
...
var _NegentropyStorageVector = class _NegentropyStorageVector {
  ...
  insertHex(timestamp, idHex) {
    ...
    this.insert(timestamp, hexToBytes3(idHex));
  }
  /**
    ...
    while (reader.remaining > 0) {
      ...
      if (mode === Mode.Skip) {
        ...
        for (let i = 0; i < numIds; i++) {
          ...
          theirElems.set(bytesToHex3(id), id);
        }
        this.storage.iterate(lower, upper, (item) => {
          const k = bytesToHex3(item.id);
          if (!theirElems.has(k)) {
            if (this.isInitiator)
              this.haveIds.push(k);
          } else {
...
var _RelayWebSocket = class _RelayWebSocket {
  ...
  async handleNegOpen(session, message) {
    ...
    try {
      ...
      const result = neg.reconcile(hexToBytes3(initialMessage));
  ...
  async handleNegMsg(session, message) {
    ...
    try {
      ...
      const result = negSession.neg.reconcile(hexToBytes3(theirMessage));
      this.sendNegMsg(session.webSocket, subId, bytesToHex3(result.message));
    } catch (error) {
...
function ensureServiceDatabase(db) {
  if (!initPromise) {
    initPromise = (async () => {
      const session = db.withSession("first-primary");
      await session.prepare(
...
__name(ensureServiceDatabase, "ensureServiceDatabase");
...
var service_worker_default = {
  async fetch(request, env, _ctx) {
    ...
    if (url.pathname === "/api/health") {
      return json2({ status: "ok", service: "sip-relay-deploy", time: Math.floor(Date.now() / 1e3) });
    }
    ...
    if (url.pathname.startsWith("/api/service/")) {
      ...
      if (!mapped.RELAY_DATABASE) {
        return json2({ error: "SERVICE_DATABASE binding missing (see wrangler.service.toml)" }, 500);
      }
